...
# from chainer_gan_lib.common.sn.sn_linear import SNLinear
# from chainer_gan_lib.common.sn.sn_convolution_2d import SNConvolution2D
...
class DCGANGenerator(chainer.Chain):
    def __init__(self, n_hidden=128, bottom_width=4, ch=512, wscale=0.02,
                 z_distribution="uniform", hidden_activation=F.relu, output_activation=F.tanh, use_bn=True, out_channel=3):
...
#             self.dc4 = L.Deconvolution2D(ch // 8, 3, 3, 1, 1, initialW=w)
            self.dc4 = L.Deconvolution2D(ch // 8, out_channel, 3, 1, 1, initialW=w)
            if self.use_bn:
...
class DCGANDiscriminator(chainer.Chain):
    def __init__(self, bottom_width=4, ch=512, wscale=0.02, output_dim=1, in_channel=3):
        w = chainer.initializers.Normal(wscale)
        super(DCGANDiscriminator, self).__init__()
        with self.init_scope():
#             self.c0_0 = L.Convolution2D(3, ch // 8, 3, 1, 1, initialW=w)
            self.c0_0 = L.Convolution2D(in_channel, ch // 8, 3, 1, 1, initialW=w)
